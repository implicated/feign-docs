...
最初，Feign 是由 Netflix 创建并发布的，作为其 Netflix OSS 项目的一部分。后来 Netflix 决定在内部停止使用 Feign，并停止开发。由于这一决定，Netflix 在一个名为 https://github.com/OpenFeign[OpenFeign^] 的新项目下将 Feign 完全转移到了开源社区。Feign 项目被贡献给了开源组织，于是才有了我们今天使用的 OpenFeign 组件。
